...
mod tests{
    ...
    fn test_ipv4_packet_parsing() {
        ...
        match super::packet_header(&packet) {
            super::nom::IResult::Done(_, packet) => {
                ...
                assert_eq!(expected_address, packet.src_addr.ipv4_address.unwrap());
                expected_address = Ipv4Addr::new(10,0,1,20);
                assert_eq!(expected_address, packet.dst_addr.ipv4_address.unwrap());
    ...
    fn test_ipv6_packet_parsing() {
        let mut packet = [
            0x00, 0x00, 0x00, 0x11, 0x25, 0x82, 0x95, 0xb5, 0x00, 0xd0, 0x09, 0xe3, 0xe8, 0xde, 0x86, 0xdd, 0x60, 0x00,
        ...
        match packet_header(&packet) {
            super::nom::IResult::Done(_, parsed) => {
                println!("{:?}", parsed);

                assert_eq!(super::IPVersion::IPV6, parsed.version);

                let mut expected_address = "2001:6f8:900:7c0::2".to_string();
                assert_eq!(expected_address, parsed.dst_addr.ip_address());

                expected_address = "2001:6f8:102d:0:2d0:9ff:fee3:e8de".to_string();
                assert_eq!(expected_address, parsed.src_addr.ip_address());
...
fn version_padding(input: &[u8], version: u8) -> nom::IResult<&[u8], Option<&str>> {
...
fn parse_ip(input: &[u8], version: u8) -> nom::IResult<&[u8], IpAddress> {
    if version == 0x08 {
        chain!(input,
            ip: ipv4_parser,
            || {
...
// fn optional_ipv4_parser(input: &[u8], version: u8) -> nom::IResult<&u8, Option<Ipv4Addr>> {
//     if version == 0x08 {
//         chain!(input,
//             ip: ipv4_parser,
//             || {
